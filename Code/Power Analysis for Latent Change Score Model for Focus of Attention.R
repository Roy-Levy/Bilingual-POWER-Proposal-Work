...
repeat{
  ...
  # print(paste0("Conducting rep ", which.rep, ", Total reps tried = ", total.tried.reps))
    ...
    #source.code.file <- "Univariate_Latent_Variable_3TimePoints_LatentGrowth_FOA.R"
    R.code.file <- "Univariate_Latent_Variable_3TimePoints_LatentGrowth_FOA.R"
  ...
  p.test.of.mean.of.slope[which.rep] <- 
      as.numeric(parameterEstimates(fitted.model) %>%
        ...
        filter(lhs=="sFOAVSP", op=="~1") %>% # select for this row
